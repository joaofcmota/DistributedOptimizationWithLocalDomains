function [X, vars_prob, varargout] = GradientMethod(n, Lipschitz, ...
    vars_prob, vars_network, varargin) 

% [X, vars_prob, varargout] = GradientMethod(n, Lipschitz, ...
%   vars_prob, vars_network, varargin)
%
...
% implementation of the gradient algorithm directly applied to (1). Each
...
%
% The remaining inputs and outputs of GradientMethod are now explained.
%
% Inputs:
...
%                           criteria are turned off: GradientMethod terminates 
...
optargs = nargin - 4;
if optargs > 1
    error('Number of input arguments exceeds the maximum value. Please type ''help GradientMethod''.');
end
if optargs == 1
    if ~isstruct(varargin{1})
        error('Optional input should be a struct. Please type ''help GradientMethod''.');
    end
end
if Lipschitz <= 0
    error('Lipschitz constant (2nd argument of GradientMethod) is not positive. Please type ''help GradientMethod''.');
end
if ~isstruct(vars_prob)
    error('vars_prob (3rd argument of GradientMethod) should be a struct. Please type ''help GradientMethod''.');
end
if sum(isfield(vars_prob, {'components', 'centers', 'gradients', 'proj_constraints'})) ~= 4
    error('vars_prob (3rd argument of GradientMethod) should contain the fields ''components'', ''centers'', ''gradients'', and ''proj_constraints''. Please type ''help GradientMethod''.');
end
if ~isstruct(vars_network)
    error('vars_network 4th argument of GradientMethod) should be a struct. Please type ''help GradientMethod''.');
end
if sum(isfield(vars_network, {'P', 'neighbors'})) ~= 2
    error('Fieldnames of the struct vars_network are not correct. Please type ''help GradientMethod''.');
end
nout = max(nargout,1)-2;
if nout > 1
    error('Number of outputs is greater than the maximum. Please type ''help GradientMethod''.'); 
end
if nout == 1
    OPT_OUTPUT = 1; 
end
...
if length(components) ~= P
    error('Size of components is not P. Please type ''help GradientMethod''.');
end

if length(centers) ~= n
    error('Size of centers is not n. PLease type ''helop GradientMethod''.')
end
% =========================================================================
...
% Struct with the internal variables of GradientMethod. 
vars = struct( 'Sp', {Sp}, ...
...
for k = 1 : MAX_ITER    
    ...
    for p = 1 : P
        ...
        if ~Stop(p)              % Only compute if node has not terminated            
            [Grads{p}, vars_prob] = gradients(p, X{p}, vars_prob);
        end
	...
	center_x_l = centers(l);
        ...
        for i = 1 : len_V_l
            ...
            sum_grad_x_l = sum_grad_x_l + Grads{node}(ind_x_l_in_node);
        end
        
        point_to_proj = X{center_x_l}(ind_x_l_in_center) - alpha*sum_grad_x_l;
        [new_x_l, vars_prob] = proj_constraints(l, point_to_proj, vars_prob);
        
        % Change x_l in all nodes in the induced subgraph
        for i = 1 : len_V_l
            node = induced_subgraph_x_l(i);
            ind_x_l_in_node = (components{node} == l);
    ...
    Stop = GradientMethod_stopping_criterion(Stop, vars_network, vars, vars_prob, k);
...
function [Stop] = GradientMethod_stopping_criterion(Stop, vars_network, vars, vars_prob, k)
