function [X, vars_prob, varargout] = KekatosADMM(n, vars_prob, vars_network,...
    varargin)

% [X, vars_prob, varargout] = KekatosADMM(n, vars_prob, vars_network,...
%  varargin)
%
...
% function in (1) is associated to a node of a network with P nodes. fp is
% the function of node p and it depends only on some components of x; this
% is indicated by the set Sp: the indices of x node p depends on. This
% function simulates a distributed scenario for solving (1) with the
% 2-block ADMM-based algorithm proposed in
%
%        V. Kekatos, G. Giannakis, Distributed Robust Power System State 
%        Estimation, IEEE Transactions on Power Systems, Vol. 28, No. 2,
%        2013
%
...
% inputs. The user gives the handler of the function solving (2) through 
% the input vars_prob. This input is a struct that can be used to store 
% information/variables relevant to solve (1), but it should contain the 
% fields 'handler' and 'components', explained next.
%
...
% variables.
%
% The remaining inputs and outputs of KekatosADMM are now explained.
%
% Inputs:
...
%                           criteria are turned off: KekatosADMM 
...
optargs = nargin - 3;
if optargs > 1
    error('Number of input arguments exceeds the maximum value. Please type ''help KekatosADMM''.');
end
if optargs == 1
    if ~isstruct(varargin{1})
        error('Optional input should be a struct. Please type ''help KekatosADMM''.');
    end
end
if ~isstruct(vars_prob)
    error('vars_prob (2nd argument of KekatosADMM) should be a struct. Please type ''help KekatosADMM''.');
end
if sum(isfield(vars_prob, {'handler', 'components'})) ~= 2
    error('vars_prob (2nd argument of KekatosADMM) should contain the field ''handler'' and the field ''components''. Please type ''help KekatosADMM''.');
end
if ~isstruct(vars_network)
    error('vars_network (3rd argument of KekatosADMM) should be a struct. Please type ''help KekatosADMM''.');
end
if sum(isfield(vars_network, {'P', 'neighbors'})) ~= 2
    error('Fieldnames of the struct vars_network are not correct. Please type ''help KekatosADMM''.');
end
nout = max(nargout,1)-2;
if nout > 1
    error('Number of outputs is greater than the maximum. Please type ''help KekatosADMM''.'); 
end
if nout == 1
    OPT_OUTPUT = 1; 
end
...
if length(components) ~= P
    error('Size of components is not P. Please type ''help KekatosADMM''.');
end
% =========================================================================
...
% Struct with the internal variables of KekatosADMM. 
vars = struct( 'Sp', {Sp}, ...
...
for k = 1 : MAX_ITER    
    ...
    vars.X_prev = X;
    [vars, vars_prob] = ADMM2block_compute_gradient_lagr(vars, vars_prob, vars_network);
    Diff = vars.Diff;
    X = vars.X;
    ...
    for p = 1 : P
        ...
        if Stop(p) == 0  
            U{p} = U{p} + (rho/2)*Diff{p};
        end
    end
    vars.U = U;
                                              
    % Stopping criterion for the outer loop
    Stop = ADMM2block_stopping_criterion(Stop, vars_network, vars, vars_prob, k);
...
function [vars, vars_prob] = ADMM2block_compute_gradient_lagr(vars, vars_prob, vars_network)
...
for p = 1 : P
    ...
    if ~Stop(p)              % Only compute if not has not terminated
        ...
        % Determine "sum of the X's of the neighbors" + length(neighbs_Vl)*
        % *x_l^p for each component l 
        for ind_l = 1 : Sp(p)
            l = comp_p(ind_l);
            ...
            D_pl = length(neighbs_Vl);
            v_aux(ind_l) = D_pl*X{p}(ind_l) + sum_neighbs_l;
            c_aux(ind_l) = D_pl;
        end
...
%         new_error = sqrt(new_error/norm(x_opt)^2);
    ...
    error_iterations = [error_iterations, new_error];          %figure(1);clf;semilogy(error_iterations);drawnow;
...
function [Stop] = ADMM2block_stopping_criterion(Stop, vars_network, vars, vars_prob, k)
