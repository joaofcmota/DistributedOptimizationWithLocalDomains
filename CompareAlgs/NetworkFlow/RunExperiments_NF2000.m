...
FILE_SAVE = 'Results/NetFlow_Barab2000.mat';
...
rhos_DADMM   = [0.06 0.08 0.10];
rhos_Kekatos = [0.10 0.12 0.14];
...
for i_rhos = 1 : len_rhos
    ...
    % Optional input
    ops = struct('rho', {rhos_Kekatos(i_rhos)}, ...
        'max_iter', {max_communications}, ...        
        'x_opt', {solution}, ...
        'eps_opt', {eps_opt} ...
        );    
    ...
    if iter_for_errors(pos_iter_errors,2) < best_iter_KekatosADMM
        ...
        best_rhos_KekatosADMM = rhos_Kekatos(i_rhos);
...
for i_Lip = 1 : len_Lips
    ...
    % It does not reach 1e-4, se record it here
    errors_NesterovMethod = error_iterations;
    best_Lips_NesterovMethod = Lips(i_Lip);
    iter_for_errors_NesterovMethod = iter_for_errors(pos_iter_errors,2);
end
...
% =========================================================================
% Save data
save(FILE_SAVE, 'rhos_DADMM', 'rhos_Kekatos', 'Lips', 'max_communications', 'eps_opt', ...
...
legend('DADMMp', 'Kekatos', 'Nesterov')
drawnow;
